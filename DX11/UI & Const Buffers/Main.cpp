	...
	_In_ int nCmdShow)					// How the window should be shown (we ignore this)
{
	...
	// Create the Game object using the app handle we got from WinMain
	Game dxGame(hInstance);

	// Result variable for function calls below
	HRESULT hr = S_OK;

	// Attempt to create the window for our program, and exit early if something failed
	hr = dxGame.InitWindow();
	if(FAILED(hr)) return hr;

	// Attempt to initialize Direct3D, and exit early if something failed
