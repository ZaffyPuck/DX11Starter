...
void Game::LoadShaders()
{
	...
	std::shared_ptr<SimplePixelShader> customPixelShader1 = std::make_shared<SimplePixelShader>(
		device, context, FixPath(L"CustomPS1.cso").c_str()); // match file name?
	std::shared_ptr<SimplePixelShader> 	customPixelShader2 = std::make_shared<SimplePixelShader>(
		device, context, FixPath(L"CustomPS2.cso").c_str());
	pixelShaders.push_back(pixelShader);
	pixelShaders.push_back(skyPS);
	pixelShaders.push_back(normalMapPS);
	pixelShaders.push_back(customPixelShader1);
...
{
	std::shared_ptr<Material> matSmooth = std::make_shared<Material>(
		white3, vertexShaders[0], pixelShaders[0], 0.0f, false, 
		DirectX::XMFLOAT2(1, 1), DirectX::XMFLOAT2(0,0)); // No spec map, shiny
	std::shared_ptr<Material> matRough = std::make_shared<Material>(
		white3, vertexShaders[0], pixelShaders[0], 1.0f, false,
		DirectX::XMFLOAT2(1, 1), DirectX::XMFLOAT2(0, 0)); // No spec map, rough
	std::shared_ptr<Material> matSpecularSmooth = std::make_shared<Material>(
		white3, vertexShaders[0], pixelShaders[0], 0.0f, true, // spec map, shiny 
		DirectX::XMFLOAT2(1, 1), DirectX::XMFLOAT2(0, 0));
	std::shared_ptr<Material> matSpecularRough = std::make_shared<Material>(
		white3, vertexShaders[0], pixelShaders[0], 1.0f, true, // spec map, rough 
		DirectX::XMFLOAT2(1, 1), DirectX::XMFLOAT2(0, 0));
	std::shared_ptr<Material> matNormalSmooth = std::make_shared<Material>(
		white3, vertexShaders[2], pixelShaders[2], 0.0f, false, // normal map, no spec map, shiny 
		DirectX::XMFLOAT2(1,1), DirectX::XMFLOAT2(0, 0));
	std::shared_ptr<Material> matNormalRough = std::make_shared<Material>(
		white3, vertexShaders[2], pixelShaders[2], 0.8f, false, // normal map, no spec map, rough/diffused
		DirectX::XMFLOAT2(1, 1), DirectX::XMFLOAT2(0, 0));
	materials.push_back(matSmooth);
	materials.push_back(matRough);
	materials.push_back(matSpecularSmooth);
	materials.push_back(matSpecularRough);
	materials.push_back(matNormalSmooth);
	materials.push_back(matNormalRough);
}
/// <summary>
...
void Game::LoadTextures()
{
	...
	// Key
	// Displacement = normals/height map
	// Metalness = specular

	// Load textures
	Microsoft::WRL::ComPtr<ID3D11ShaderResourceView> flatNormSRV;
	HRESULT flatNormTResult = CreateWICTextureFromFile(device.Get(), context.Get(),
		L"../External/Textures/flat_normals.png", 0, flatNormSRV.GetAddressOf());
	...
	HRESULT woodSpecTResult = CreateWICTextureFromFile(device.Get(), context.Get(), 
		L"../External/Textures/weathered_brown_planks/weathered_brown_planks_rough_4k.jpg", 0, woodSpecSRV.GetAddressOf()); // not spec - rough map?
	...
	HRESULT tileSpecTResult = CreateWICTextureFromFile(device.Get(), context.Get(),
		L"../External/Textures/stone_tiles/stone_tiles_disp_4k.png", 0, tileSpecSRV.GetAddressOf()); // not spec - displacement?

	Microsoft::WRL::ComPtr<ID3D11ShaderResourceView> metalSRV;
	Microsoft::WRL::ComPtr<ID3D11ShaderResourceView> metalSpecSRV;
	Microsoft::WRL::ComPtr<ID3D11ShaderResourceView> metalNormSRV;
	HRESULT metalTResult = CreateWICTextureFromFile(device.Get(), context.Get(),
		L"../External/Textures/metal/Metal_Color.png", 0, metalSRV.GetAddressOf());
	HRESULT metalSpecTResult = CreateWICTextureFromFile(device.Get(), context.Get(),
		L"../External/Textures/metal/Metal_Metalness.png", 0, metalSpecSRV.GetAddressOf());
	HRESULT metalNormTResult = CreateWICTextureFromFile(device.Get(), context.Get(),
		L"../External/Textures/metal/Metal_Metalness.png", 0, metalNormSRV.GetAddressOf());

	Microsoft::WRL::ComPtr<ID3D11ShaderResourceView> cushionSRV;
	Microsoft::WRL::ComPtr<ID3D11ShaderResourceView> cushionNormSRV;
	HRESULT cushionTResult = CreateWICTextureFromFile(device.Get(), context.Get(),
		L"../External/Textures/cushion/cushion.png", 0, cushionSRV.GetAddressOf());
	HRESULT cushionNormTResult = CreateWICTextureFromFile(device.Get(), context.Get(),
		L"../External/Textures/cushion/cushion_normals.png", 0, cushionNormSRV.GetAddressOf());

	// Bind textures to mats
	materials[0]->AddSampler("BasicSampler", sampler);
	materials[0]->AddTextureSRV("SurfaceTexture", woodSRV);
	materials[0]->AddTextureSRV("SpecularMap", woodSpecSRV); // not used
	materials[0]->AddTextureSRV("NormalMap", flatNormSRV); // basic/flat

	materials[1]->AddSampler("BasicSampler", sampler);
	materials[1]->AddTextureSRV("SurfaceTexture", tileSRV);
	materials[1]->AddTextureSRV("SpecularMap", tileSpecSRV); // not used
	materials[1]->AddTextureSRV("NormalMap", flatNormSRV); // basic/flat

	materials[2]->AddSampler("BasicSampler", sampler);
	materials[2]->AddTextureSRV("SurfaceTexture", metalSRV);
	materials[2]->AddTextureSRV("SpecularMap", metalSpecSRV); // using
	materials[2]->AddTextureSRV("NormalMap", flatNormSRV); // basic/flat

	materials[3]->AddSampler("BasicSampler", sampler);
	materials[3]->AddTextureSRV("SurfaceTexture", metalSRV);
	materials[3]->AddTextureSRV("SpecularMap", metalSpecSRV); // using
	materials[3]->AddTextureSRV("NormalMap", flatNormSRV); // basic/flat

	materials[4]->AddSampler("BasicSampler", sampler);
	materials[4]->AddTextureSRV("SurfaceTexture", cushionSRV);
	materials[4]->AddTextureSRV("NormalMap", cushionNormSRV);

	//materials[5]->AddSampler("BasicSampler", sampler);
	//materials[5]->AddTextureSRV("SurfaceTexture", cushionSRV);
	//materials[5]->AddTextureSRV("NormalMap", cushionNormSRV);

	materials[5]->AddSampler("BasicSampler", sampler);
	materials[5]->AddTextureSRV("SurfaceTexture", metalSRV);
	materials[5]->AddTextureSRV("SpecularMap", metalSpecSRV); // using
	materials[5]->AddTextureSRV("NormalMap", metalNormSRV); // basic/flat
...
void Game::CreateEntities()
{
	...
	std::shared_ptr<Entity> e1 = std::make_shared<Entity>(meshes[0], materials[4]);
	std::shared_ptr<Entity> e2 = std::make_shared<Entity>(meshes[1], materials[4]);
	std::shared_ptr<Entity> e3 = std::make_shared<Entity>(meshes[5], materials[4]);
	std::shared_ptr<Entity> e4 = std::make_shared<Entity>(meshes[0], materials[5]);
	std::shared_ptr<Entity> e5 = std::make_shared<Entity>(meshes[1], materials[5]);
	std::shared_ptr<Entity> e6 = std::make_shared<Entity>(meshes[5], materials[5]);
