...
void Game::Init()
{
	...
	InitPostProcessing();	// Init Post Processing
	...
	InitShadowMapTexture();	// Init Shadow Map
}
/// <summary>
...
void Game::LoadShaders()
{
	...
	std::shared_ptr<SimpleVertexShader> shadowMapVS = std::make_shared<SimpleVertexShader>(
		device, context, FixPath(L"ShadowMapVS.cso").c_str());
	vertexShaders.push_back(shadowMapVS);

	// Post processing shaders
	// -- Full Screen shader
	fullScreenVS = std::make_shared<SimpleVertexShader>(
		device, context, FixPath(L"FullScreenVS.cso").c_str());
	vertexShaders.push_back(fullScreenVS);

	// -- Box Blur shaders
	boxBlurPPPS = std::make_shared<SimplePixelShader>(
		device, context, FixPath(L"BoxBlurPPPS.cso").c_str());
	pixelShaders.push_back(boxBlurPPPS);
}
/// <summary>
...
void Game::InitPostProcessing()
{
	...
	D3D11_SAMPLER_DESC postProcessSampDesc = {};
	postProcessSampDesc.AddressU = D3D11_TEXTURE_ADDRESS_CLAMP; // clamps to edges of screen instead of wrap
	postProcessSampDesc.AddressV = D3D11_TEXTURE_ADDRESS_CLAMP;
	postProcessSampDesc.AddressW = D3D11_TEXTURE_ADDRESS_CLAMP;
	postProcessSampDesc.Filter = D3D11_FILTER_MIN_MAG_MIP_LINEAR;
	postProcessSampDesc.MaxLOD = D3D11_FLOAT32_MAX;
	device->CreateSamplerState(&postProcessSampDesc, postProcessSampler.GetAddressOf());
	...
	Microsoft::WRL::ComPtr<ID3D11Texture2D> postProcessTexture; // Create the resource (no need to track it after the views are created below)
	...
	// Create the Shader Resource View by passing it a null description for the SRV, 
	// we get a "default" SRV that has access to the entire resource
...
{
	std::shared_ptr<Entity> floor = std::make_shared<Entity>(meshes[0], materials[7]);
	...
	// dont need init position since objects are updated
	//float x = 0.0f;
	//float y = 0.0f;
	//float deltaX = 2.0f;
	//float deltaY = 0.0f;
...
void Game::CreateLights()
{
	...
	dLight6.direction = DirectX::XMFLOAT3(0.0f, -1.0f, 0.01f); // 0s dont work?
	dLight6.position = DirectX::XMFLOAT3(0.0f, 40.0f, 0.0f); // how far should it idealy be?
	dLight6.color = white3;
	dLight6.intensity = 0.3f;
	dLight6.castsShadows = 1; // 0 = false, 1 = true
...
void Game::InitShadowMapTexture()
{
	...
	// View Matrix - look from light perspective - bind to light 6
	XMMATRIX lightView = XMMatrixLookAtLH(
		XMLoadFloat3(&lights[5].position),
		XMLoadFloat3(&lights[5].direction),
		XMVectorSet(0, 1, 0, 0)); // global up
	XMStoreFloat4x4(&shadowViewMatrix, lightView);

	// Proj Matrix - normalize
...
void Game::OnResize()
{
	ResetRenderTargets();// Screen sized render targets need to be recreated
...
void Game::Update(float deltaTime, float totalTime)
{
	...
	int objAmount = 8;
	float objWidth = 2.0f;
	float xOffset = (objAmount * objWidth) * -0.5f;
	float yOffset = 3.0f;
	float ySinOffset = XM_2PI/ objAmount;
	float sine = (float)sin(totalTime);
	float cosine = (float)cos(totalTime);
	entities[1]->GetTransform()->SetPosition(objWidth * 0 + 1, sin(totalTime) + 2.0f, 0.0f);

	for (int i = 1; i < objAmount+1; i++)
	{
		entities[i]->GetTransform()->SetPosition(objWidth * i + xOffset, sin(totalTime + ySinOffset * i) + yOffset, 0.0f);
	}
...
void Game::Draw(float deltaTime, float totalTime)
{
	...
	// Pre-Render
	context->ClearRenderTargetView(postProcessRTV.Get(), clearColor);
	...
	// Activate Depth Buffer and render opaque objects to the screen
	for (auto& entity : entities) {
		std::shared_ptr<SimpleVertexShader> vs = entity->GetMaterial()->GetVertexShader();
		vs->SetMatrix4x4("shadowViewMatrix", shadowViewMatrix);
		vs->SetMatrix4x4("shadowProjectionMatrix", shadowProjectionMatrix);
	...
	// Render transparent objects and draw lights when implemented

	// Post-Render
	context->OMSetRenderTargets(1, backBufferRTV.GetAddressOf(), 0);
	// Turn OFF vertex and index buffers since we'll be using the full-screen triangle trick
	UINT stride = sizeof(Vertex);
	UINT offset = 0;
	ID3D11Buffer* nothing = 0;
	context->IASetIndexBuffer(0, DXGI_FORMAT_R32_UINT, 0);
	context->IASetVertexBuffers(0, 1, &nothing, &stride, &offset);
	context->PSSetSamplers(0, 1, postProcessSampler.GetAddressOf());

	// Activate shaders and bind resources and set any required cbuffer data (not shown)
	fullScreenVS->SetShader();
	fullScreenVS->CopyAllBufferData();
	boxBlurPPPS->SetShader();
	boxBlurPPPS->SetInt("blurRadius", blurRadius);
	boxBlurPPPS->SetFloat("pixelWidth", (1.0f / windowWidth));
	boxBlurPPPS->SetFloat("pixelHeight", (1.0f / windowHeight));
	boxBlurPPPS->SetShaderResourceView("Pixels", postProcessSRV.Get());
	boxBlurPPPS->SetSamplerState("ClampSampler", postProcessSampler.Get());
	boxBlurPPPS->CopyAllBufferData();
	...
	// Present the back buffer to the user by putting the results of what we've drawn onto the window
...
{
	// For each shadow casting light, render the entire scene using lights shadow map as depth bugger
	context->OMSetRenderTargets(0, 0, shadowDSV.Get()); // Set up the output merger stage
	// Clear the shadow depth buffer
	context->ClearDepthStencilView(shadowDSV.Get(), D3D11_CLEAR_DEPTH, 1.0f, 0); // Clear the shadow map
	...
	//Turn shadow VS on and PS off
	vertexShaders[3]->SetShader();
	vertexShaders[3]->SetMatrix4x4("viewMatrix", shadowViewMatrix);
	vertexShaders[3]->SetMatrix4x4("projectionMatrix", shadowProjectionMatrix);
	context->PSSetShader(0, 0, 0); // Deactivate pixel shader

	// Loop and draw all entities
	for (auto& entity : entities)
	{
		vertexShaders[3]->SetMatrix4x4("worldMatrix", entity->GetTransform()->GetWorldMatrix());
		vertexShaders[3]->CopyAllBufferData();

		entity->GetMesh()->SetBuffersAndDraw(context); // Draw the mesh directly to avoid the entity's material
	}
...
void Game::ResetRenderTargets()
{
	...
	D3D11_TEXTURE2D_DESC textureDesc = {};
	textureDesc.Width = windowWidth; // update
	textureDesc.Height = windowHeight; // update
	textureDesc.ArraySize = 1;
	textureDesc.BindFlags = D3D11_BIND_RENDER_TARGET | D3D11_BIND_SHADER_RESOURCE;
	textureDesc.CPUAccessFlags = 0;
	textureDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
...
void Game::IGRun() {
	...
	if (ImGui::TreeNode("Lights"))
	{
		for (int i = 0; i < lights.size(); i++)
		{
			...
			if (ImGui::TreeNode("Light Node", "Light %d", i))
			{
				...
				if (ImGui::DragFloat3("Direction", &lDir.x, 0.01f))
				{
					lights[i].direction = lDir;

					// Only works for light 6
					if (i == 5)
					{
						int lightDistance = 50; // how far should it idealy be?
						//XMFLOAT3 signedLightDirection = XMFLOAT3();
						//signedLightDirection.x = lights[i].direction.x >= 0 ? -1.0f : 1.0f;
						//signedLightDirection.y = lights[i].direction.y >= 0 ? -1.0f : 1.0f;
						//signedLightDirection.z = lights[i].direction.z >= 0 ? -1.0f : 1.0f;

						// linked direction to calc position - has weird behavior - because I want just the direction, not magnitude
						lights[i].position = DirectX::XMFLOAT3(
							-lights[i].direction.x * lightDistance,
							-lights[i].direction.y * lightDistance,
							-lights[i].direction.z * lightDistance);

						// might need to normalize direction then multiply by light distance to avoid weirdness
				...
				else if (lType == LIGHT_TYPE_SPOT)
				{
					...
					XMFLOAT2 lPadding = lights[i].padding;
	...
	{
		ImGui::Spacing();
		if (ImGui::DragFloat("Light Projection Size", &lightProjectionSize, 0.01f))
		{
			if (lightProjectionSize <= 0) { lightProjectionSize = 0.01f; }
			XMMATRIX lightProjection = XMMatrixOrthographicLH(
				lightProjectionSize,
				lightProjectionSize,
				1.0f,
		...
		{
			ImGui::Image(shadowSRV.Get(), ImVec2(100, 100));
			ImGui::TreePop();
		}
	...
	if (ImGui::TreeNode("Post Processing"))
	{
		...
		int blur = blurRadius;
		if (ImGui::DragInt("Blur Radius", &blur, 0.01f)) blurRadius = blur;

		if (ImGui::TreeNode("Sample"))
		{
			ImGui::Image(postProcessSRV.Get(), ImVec2(100, 100));
			ImGui::TreePop();
		}
