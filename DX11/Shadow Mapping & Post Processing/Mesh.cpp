...
{
	// Calc tangents
	CalculateTangents(vertexArray, vertexAmount, indexArray, indexAmount);
...
// --------------------------------------------------------
void Mesh::CalculateTangents(Vertex* vertexArray, size_t vertexAmount, unsigned int* indexArray, size_t indexAmount)
{
	// Reset tangents
	for (int i = 0; i < vertexAmount; i++)
	{
		vertexArray[i].tangent = DirectX::XMFLOAT3(0, 0, 0);
	}
	// Calculate tangents one whole triangle at a time
	for (int i = 0; i < indexAmount;)
	{
		// Grab indices and vertices of first triangle
		unsigned int i1 = indexArray[i++];
		unsigned int i2 = indexArray[i++];
		unsigned int i3 = indexArray[i++];
		Vertex* v1 = &vertexArray[i1];
		Vertex* v2 = &vertexArray[i2];
		Vertex* v3 = &vertexArray[i3];
	...
	for (int i = 0; i < vertexAmount; i++)
	{
		...
		DirectX::XMVECTOR normal = DirectX::XMLoadFloat3(&vertexArray[i].normal);
		DirectX::XMVECTOR tangent = DirectX::XMLoadFloat3(&vertexArray[i].tangent);
		// Use Gram-Schmidt orthonormalize to ensure
		// the normal and tangent are exactly 90 degrees apart
		DirectX::XMVECTOR dot = DirectX::XMVector3Dot(normal, tangent);
		DirectX::XMVECTOR projNormal = DirectX::XMVectorMultiply(normal, dot);
		tangent = DirectX::XMVectorSubtract(projNormal, tangent);
		// Store the tangent
		DirectX::XMStoreFloat3(&vertexArray[i].tangent, tangent);
	}
}