...
{
	std::shared_ptr<Material> matSmooth = std::make_shared<Material>(white3, vertexShader, pixelShader, 0.0f);
	std::shared_ptr<Material> matRough = std::make_shared<Material>(white3, vertexShader, pixelShader, 0.99f); // 1.0f breaks it
	std::shared_ptr<Material> mat2 = std::make_shared<Material>(white3, vertexShader, customPixelShader, 0.0f); // color doesnt do anything
	std::shared_ptr<Material> mat3 = std::make_shared<Material>(white3, vertexShader, customPixelShader2, 0.0f);// color doesnt do anything
...
void Game::CreateEntities()
{
	...
	std::shared_ptr<Entity> e4 = std::make_shared<Entity>(meshes[3], materials[0]);
	std::shared_ptr<Entity> e5 = std::make_shared<Entity>(meshes[4], materials[0]);
	std::shared_ptr<Entity> e6 = std::make_shared<Entity>(meshes[5], materials[0]);
	std::shared_ptr<Entity> e7 = std::make_shared<Entity>(meshes[6], materials[0]);
	std::shared_ptr<Entity> e8 = std::make_shared<Entity>(meshes[5], materials[1]);
	std::shared_ptr<Entity> e9 = std::make_shared<Entity>(meshes[6], materials[1]);
	...
	entities[7]->GetTransform()->SetPosition(x, 0.0f, 0.0f); x += 2;
	entities[8]->GetTransform()->SetPosition(x, 0.0f, 0.0f); x+=2;
}
void Game::CreateLights()
{
	...
	dLight1.color = blue3;
	dLight1.intensity = 0.5f;

	Light dLight2 = {};
	dLight2.type = LIGHT_TYPE_DIRECTIONAL;
	dLight2.direction = DirectX::XMFLOAT3(+1.0f, 0.0f, 0.0f);
	dLight2.color = purple3;
	dLight2.intensity = 0.4f;

	Light dLight3 = {};
	dLight3.type = LIGHT_TYPE_DIRECTIONAL;
	dLight3.direction = DirectX::XMFLOAT3(0.0f, 0.5f, -1.0f);
	dLight3.color = yellow3;
	dLight3.intensity = 0.5f;

	Light dLight4 = {};
	dLight4.type = LIGHT_TYPE_DIRECTIONAL;
	dLight4.direction = DirectX::XMFLOAT3(0.0f, -0.5f, +1.0f);
	...
	dLight5.color = orange3;
	dLight5.intensity = 0.5f;
	...
	pLight1.position = DirectX::XMFLOAT3(0.0f, 3.0f, 0.0f);
	pLight1.range = 8.0f;
	...
	pLight2.position = DirectX::XMFLOAT3(10.0f, 2.0f, 0.0f);
	pLight2.range = 4.0f;
...
void Game::Update(float deltaTime, float totalTime)
{
	...
	entities[1]->GetTransform()->Rotate(deltaTime * rotationSpeed, 0.0f, 0.0f);
	entities[2]->GetTransform()->Rotate(0.0f, deltaTime * rotationSpeed, 0.0f);
	entities[3]->GetTransform()->Rotate(0.0f, 0.0f, deltaTime * rotationSpeed);
	entities[4]->GetTransform()->Rotate(deltaTime * rotationSpeed, 0.0f, 0.0f);
	entities[5]->GetTransform()->Rotate(0.0f, deltaTime * rotationSpeed, 0.0f);
		...
		for (int i = 0; i < entities.size(); i++)
		{
			...
			if (ImGui::TreeNode("Entity Node", "Entity %d", i))
			{
				...
				if (ImGui::TreeNode("Material Node", "Material", i))
				{
					...
					if (ImGui::DragFloat("Position", &roughness, 0.01f)) mat->SetRoughness(roughness);
					ImGui::Spacing();
					ImGui::TreePop();
				}
