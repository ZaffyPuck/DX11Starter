...
float SpecularPhong(Light light, float3 normal, float roughness, float3 dirToCamera)
{
    ...
    if (roughness == 1){return 0.0f;}
    else
    {
        return pow(max(dot(dirToCamera, reflection), 0), specExponent);
    }
    ...
    for (int i = 0; i < LIGHT_AMOUNT; i++)
    {
        ...
        switch (light.type)
        {
            case LIGHT_TYPE_DIRECTIONAL:
                ...
                finalColor = surfaceColor * diffusion + specular; // Don’t tint specular? // //float3 light = colorTint * (diffuse + specular); // Tint specular?
            ...
            case LIGHT_TYPE_POINT:
                ...
                finalColor = surfaceColor * diffusion + specular;
