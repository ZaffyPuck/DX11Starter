...
void RaytracingHelper::CreateRaytracingPipelineState(std::wstring raytracingShaderLibraryFile)
{
	...
	// === Closest hit shader ===
	{
		...
		closestHitExportDesc[3].Name = L"ClosestHitShadow";
		...
		closestHitLibDesc.NumExports = ARRAYSIZE(closestHitExportDesc); // NUM_HIT_GROUPS (4)
	...
	// === Hit group ===
	{
		...
		subobjects[6] = hitGroupShadow;
	}

	// === Shader config (payload) ===
	{
		D3D12_RAYTRACING_SHADER_CONFIG shaderConfigDesc = {};
		shaderConfigDesc.MaxPayloadSizeInBytes = sizeof(DirectX::XMFLOAT3) + sizeof(unsigned int) * 2 + sizeof(bool)*8;// Float3 color
		...
		subobjects[7] = shaderConfigSubObj;
	}
	...
	// === Association - Payload and shaders ===
	{
		...
		subobjects[8] = shaderPayloadAssociationObject;
	}
	...
	// === Association - Shaders and local root sig ===
	{
		...
		rootSigAssociation.pSubobjectToAssociate = &subobjects[9]; // Root sig above
		...
		subobjects[10] = rootSigAssociationSubObj;
	}
	...
	// === Global root sig ===
	{
		...
		subobjects[11] = globalRootSigSubObj;
	}
	...
	// === Pipeline config ===
	{
		...
		subobjects[12] = pipelineConfigSubObj;
	}
...
void RaytracingHelper::CreateShaderTable()
{
	...
	int alignedHitGroupCount = MAX_HIT_GROUPS_IN_SHADER_TABLE / HIT_GROUP_COUNT * HIT_GROUP_COUNT;
	for (unsigned int i = 0; i < alignedHitGroupCount; i += HIT_GROUP_COUNT)
	{
		...
		memcpy(shaderTableData, raytracingPipelineProperties->GetShaderIdentifier(L"HitGroupShadow"), D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES);
		shaderTableData += shaderTableRecordSize;
	}
...
MeshRaytracingData RaytracingHelper::CreateBottomLevelAccelerationStructureForMesh(Mesh* mesh)
{
	...
	geometryDesc.Triangles.VertexBuffer.StartAddress = mesh->GetVertexBufferResource()->GetGPUVirtualAddress();
	...
	geometryDesc.Triangles.IndexBuffer = mesh->GetIndexBufferResource()->GetGPUVirtualAddress();
	...
	dxrDevice->CreateShaderResourceView(mesh->GetIndexBufferResource().Get(), &indexSRVDesc, ib_cpu);
	...
	dxrDevice->CreateShaderResourceView(mesh->GetVertexBufferResource().Get(), &vertexSRVDesc, vb_cpu);
	...
	shaderTable->Map(0, 0, (void**)&tablePointer);
	{
		...
		tablePointer += shaderTableRecordSize * raytracingData.HitGroupIndex * HIT_GROUP_COUNT; // Skip to this hit group
		...
		// Handle 2nd hit group
		tablePointer += shaderTableRecordSize; // Jump to SRV in next record
		memcpy(tablePointer, &raytracingData.IndexbufferSRV, 8); // Copy descriptor to table

		// Handle 4th hit group
		tablePointer += shaderTableRecordSize * 2; // Jump to SRV in next record
		memcpy(tablePointer, &raytracingData.IndexbufferSRV, 8); // Copy descriptor to table
...
// --------------------------------------------------------
void RaytracingHelper::CreateTopLevelAccelerationStructureForScene(std::vector<std::shared_ptr<GameObject>> scene)
{
	if (scene.size() == 0)
		return;
	...
	for (size_t i = 0; i < scene.size(); i++)
	{
		...
		// Calc the offset to the right hit group
		MaterialType type = scene[i]->GetMaterial()->GetType();
		int hitGroupOffset = 0;
		if (type == MaterialType::Transparent)
		{
			hitGroupOffset = 1; // should be 1
		}
		else if (type == MaterialType::Emissive)
		{
			hitGroupOffset = 2;
		}
		else if (type == MaterialType::Shadow)
		{
			hitGroupOffset = 3;
		}
		...
		id.InstanceContributionToHitGroupIndex = meshBlasIndex * HIT_GROUP_COUNT + hitGroupOffset;
		...
		XMFLOAT3 color = scene[i]->GetMaterial()->GetColorTint();
		float roughness = scene[i]->GetMaterial()->GetRoughness();
		entityData[meshBlasIndex].color[id.InstanceID] = XMFLOAT4(color.x, color.y, color.z, roughness); // Using alpha channel as "roughness"
	...
	for(int i = 0; i < entityData.size(); i++)
	{
		...
		unsigned char* hitGroupPointer = tablePointer + shaderTableRecordSize * HIT_GROUP_COUNT * i;
		...
		hitGroupPointer += shaderTableRecordSize;
		memcpy(hitGroupPointer, &cbv, sizeof(D3D12_GPU_DESCRIPTOR_HANDLE)); // Shadow
	}
	shaderTable->Unmap(0, 0);
}
